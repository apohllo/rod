...
module Rod
  module Accessor
    ...
    class SequenceAccessor < Base
      ...
      # +bytes_database+.
      def initialize(property,database,bytes_database,offset)
      ...
      def save(object)
        ...
        @database.write_ulong(object_offset(object),@offset,offset)
        @database.write_ulong(object_offset(object),@offset+1,length)
      end

      # Load the value of the property of the +object+ from the database.
      def load(object)
        raise InvalidArgument.new("object",nil) if object.nil?
        offset = @database.read_ulong(object_offset(object),@offset)
        length = @database.read_ulong(object_offset(object),@offset+1)
